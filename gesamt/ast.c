...
astnode *new_function_node(char *name, int varcount, astnode *parameters, astnode *sequence) {
    ...
    enter_node->pos = varcount;

    astnode *begin_node = _new_operator_node(OP_BGN, enter_node, parameters);

    astnode *function_node = _new_operator_node(OP_FUN, begin_node, sequence);
    return function_node;
}

astnode *new_parameters_node(int position, astnode *next) {
    astnode *parameter_node = _new_operator_node(OP_PAR, NULL, NULL);
    parameter_node->pos = position;

    astnode *parameters_node = _new_operator_node(OP_PRS, parameter_node, next);
